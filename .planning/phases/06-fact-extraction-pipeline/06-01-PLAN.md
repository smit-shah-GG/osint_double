...
**CRITICAL: Wire fact_schema to its dependencies by importing Entity and Provenance schemas.**

At the top of fact_schema.py, add these explicit imports to establish the key links:
...
These imports wire the fact schema to its dependent schemas, enabling:
- ExtractedFact.entities field to use Entity type
- ExtractedFact.provenance field to use Provenance type

...
must_haves:
  ...
  key_links:
    - from: "osint_system/agents/sifters/verification/evidence_aggregator.py"
      ...
      pattern: "DOMAIN_AUTHORITY|WIRE_SERVICE"
...
1. `_get_authority_score(self, source_url: str) -> float`:
   ...
   - Apply authority scoring from source_credibility.py patterns:
     - Wire services (ap.com, reuters.com, afp.com): 0.9
     - .gov, .edu domains: 0.85
     - .org domains: 0.7
     - Known news outlets: 0.5
     - Social media (twitter.com, reddit.com): 0.3
     - Default: 0.4
...
"` succeeds
  ...
  <done>
EvidenceAggregator confirms with 1 high-authority source OR 2+ independent lower-authority, refutes with authority >= 0.7, returns PENDING otherwise
  </done>
</task>
...
**Constructor:**
- `impact_assessor: Optional[ImpactAssessor] = None` - Lazy init
...
4. **Impact re-assessment (CONFIRMED only):**
   ...
   - Add verification evidence to fact context
   - Run ImpactAssessor.assess() with enriched fact
...
"` succeeds
  ...
  <done>
Reclassifier preserves origin flags, updates confidence, re-assesses impact for CONFIRMED facts, handles ANOMALY resolution with context-dependent loser status
  </done>
</task>
...
`uv run python -m pytest tests/agents/sifters/verification/test_evidence_aggregator.py tests/agents/sifters/verification/test_reclassifier.py -v` passes with 40+ tests
  ...
  <done>
Evidence aggregation tests verify authority thresholds and confidence boosts, reclassifier tests verify origin preservation and ANOMALY resolution
  </done>
</task>

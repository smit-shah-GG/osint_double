...
g = QueryGenerator()
# Test with FOG flag - use enum value not string
...
6. **Query variant type tests:**
   ...
   - All queries have dubious_flag set (use DubiousFlag enum values, not strings)
...
**IMPORTANT:** When creating VerificationQuery objects in tests, always use DubiousFlag enum values (e.g., `DubiousFlag.PHANTOM`) not string literals (e.g., `'phantom'`). The schema expects the enum type.

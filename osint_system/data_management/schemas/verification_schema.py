...
query generation, result storage, and persistence. All Phase 8 components
...
Usage:
    from osint_system.data_management.schemas.verification_schema import (
        ...
        VerificationQuery, EvidenceEvaluation, VerificationResultRecord
    )

Note: Core schemas are defined here in the data layer to avoid circular imports.
The agent layer (agents.sifters.verification.schemas) re-exports these types.
"""
...
class VerificationResultRecord(VerificationResult):
    """Persistence wrapper for VerificationResult.

    Extends VerificationResult with storage-specific timestamps.
    All fields from VerificationResult are inherited directly.

    Provides conversion methods for round-trip serialization:
    - from_result(): Creates a record from a VerificationResult
    - to_result(): Extracts the core VerificationResult

    Follows the same pattern as FactClassification's storage approach
    in classification_schema.py.
    """

    created_at: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc),
        description="Record creation timestamp (when first stored)",
    )
    updated_at: datetime = Field(
        default_factory=lambda: datetime.now(timezone.utc),
        description="Last update timestamp (modified on re-verification or review)",
    )

    @classmethod
    def from_result(cls, result: VerificationResult) -> "VerificationResultRecord":
        """Create a storage record from a VerificationResult.

        Args:
            result: The verification result to wrap for storage.

        Returns:
            VerificationResultRecord with storage timestamps set to now.
        """
    ...
    def to_result(self) -> VerificationResult:
        """Extract the core VerificationResult without storage fields.

        Returns:
            VerificationResult with all verification-specific fields preserved.
        """
        data = self.model_dump()
        # Remove storage-specific fields not in VerificationResult
            ...
            "examples": [
                {
                    ...
                    "query_attempts": 1,
                    "queries_used": [
                        "Putin Ukraine official statement"
                    ],
                    "origin_dubious_flags": ["phantom"],
